<x-app-layout :title="__(':name bearbeiten', ['name' => $record->title])">
    <div class="container" style="max-width: 70rem;">
        <h1 class="h3 mb-3">{{ $record->title }} <small class="text-body-secondary">{{ __('bearbeiten') }}</small></h1>
        <form action="{{ route('records.update', $record) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="d-flex gap-2 mb-4">
                ...
                <a href="{{ route('records.show', $record) }}" class="btn btn-outline-secondary">{{ __('Abbrechen') }}</a>
            </div>
        </form>
    </div>
</x-app-layout>
